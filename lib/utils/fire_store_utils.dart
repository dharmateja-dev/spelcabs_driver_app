...
class FireStoreUtils {
          ...
          if (data != null) {
            ...
            if (order.status == Constant.ridePlaced) {
              if (order.acceptedDriverId != null &&
                ...
                AppLogger.debug(
                    "Order ${doc.id} already accepted by current driver, skipping.",
                    tag: "FireStoreUtils");
                continue;
              }
  ...
  static Future<bool> hasActiveRide() async {
    String uid = getCurrentUid();
    AppLogger.debug("Checking for active or unpaid rides for driver: $uid",
        tag: "FireStoreUtils");
    try {
      // 1. Check for active city rides (Active, InProgress)
      final activeCityRides = await fireStore
          .collection(CollectionName.orders)
          .where('driverId', isEqualTo: uid)
          .where('status',
      ...
      if (activeCityRides.docs.isNotEmpty) {
        AppLogger.info(
            "Driver has active city ride: ${activeCityRides.docs.first.id}",
            tag: "FireStoreUtils");
        return true;
      }

      // 2. Check for completed city rides with PENDING PAYMENT
      final unpaidCityRides = await fireStore
          .collection(CollectionName.orders)
          .where('driverId', isEqualTo: uid)
      ...
      if (unpaidCityRides.docs.isNotEmpty) {
        AppLogger.info(
            "Driver has completed city ride with pending payment: ${unpaidCityRides.docs.first.id}",
            tag: "FireStoreUtils");
        return true;
      }

      // 3. Check for active intercity/freight rides
      final activeIntercityRides = await fireStore
          .collection(CollectionName.ordersIntercity)
          .where('driverId', isEqualTo: uid)
          .where('status',
              whereIn: [Constant.rideActive, Constant.rideInProgress])
      ...
      if (activeIntercityRides.docs.isNotEmpty) {
        AppLogger.info(
            "Driver has active intercity/freight ride: ${activeIntercityRides.docs.first.id}",
            tag: "FireStoreUtils");
        return true;
      }

      // 4. Check for completed intercity/freight rides with PENDING PAYMENT
      final unpaidIntercityRides = await fireStore
          .collection(CollectionName.ordersIntercity)
          .where('driverId', isEqualTo: uid)
          .where('status', isEqualTo: Constant.rideComplete)
          .where('paymentStatus', isEqualTo: false)
      ...
      if (unpaidIntercityRides.docs.isNotEmpty) {
        AppLogger.info(
            "Driver has completed intercity ride with pending payment: ${unpaidIntercityRides.docs.first.id}",
            tag: "FireStoreUtils");
        return true;
      }

      AppLogger.debug("No active or unpaid rides found for driver: $uid",
          tag: "FireStoreUtils");
      return false;
    } catch (e) {
      AppLogger.error("Error checking for active/unpaid rides: $e",
          tag: "FireStoreUtils");
      return false;
    }
  }
