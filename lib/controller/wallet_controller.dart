...
import 'package:driver/ui/wallet/phonepeScreen.dart';
...
class WalletController extends GetxController {
  ...
  // PhonePe Payement Integration
  Future<void> phonePeMakePayment({required String amount}) async {
    ShowToastDialog.showLoader("Initiating PhonePe payment...".tr);

    if (paymentModel.value.phonePe == null ||
        paymentModel.value.phonePe!.enable != true) {
      ShowToastDialog.showToast("PhonePe payment is not enabled.".tr);
      ShowToastDialog.closeLoader();
      return;
    }

    final PhonePe phonePeConfig = paymentModel.value.phonePe!;
    final String merchantId = phonePeConfig.merchantId ?? '';
    final String saltKey = phonePeConfig.saltKey ?? '';
    final String saltIndex = phonePeConfig.saltIndex ?? '';
    final bool isSandbox = phonePeConfig.isSandbox ??
        true; // Use the isSandbox flag from the model

    if (merchantId.isEmpty || saltKey.isEmpty || saltIndex.isEmpty) {
      ShowToastDialog.showToast(
          "PhonePe credentials are not set up properly. Please contact admin."
              .tr);
      ShowToastDialog.closeLoader();
      return;
    }

    final String transactionId = Constant.getUuid(); // Unique transaction ID
    final int amountInPaise =
        (double.parse(amount) * 100).round(); // Amount in paise

    // Construct the payload as required by PhonePe
    // This is a simplified example; refer to PhonePe API docs for exact structure
    // For a real integration, this payload should ideally be generated on your backend
    // to keep sensitive keys secure.
    Map<String, dynamic> paymentPayload = {
      "merchantId": merchantId,
      "merchantTransactionId": transactionId,
      "amount": amountInPaise,
      "currency": "INR", // Assuming INR, adjust as per your app's currency
      "callbackUrl":
          "${Constant.globalUrl}payment/phonepe_callback", // Your backend callback URL
      "redirectUrl":
          "${Constant.globalUrl}payment/phonepe_redirect", // URL PhonePe redirects to after payment
      "redirectMode": "POST",
      "mobileNumber": driverUserModel.value.phoneNumber, // User's phone number
      "paymentInstrument": {
        "type": "PAY_PAGE",
      },
    };
    ...
    // PhonePe API endpoint for initiating payment
    String phonePeApiUrl = isSandbox
        ? "https://api-preprod.phonepe.com/apis/pg-ind/v1/pay" // Sandbox URL
        : "https://api.phonepe.com/apis/pg-ind/v1/pay"; // Production URL

    try {
      final response = await http.post(
        Uri.parse(phonePeApiUrl),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        log("PhonePe API Response: $responseData");
        ...
        if (responseData['success'] == true &&
          ...
          // Navigate to PhonePeScreen to open the WebView
          bool? paymentSuccess = await Get.to(() => PhonePeScreen(
                initialURl: redirectUrl,
                successUrl:
                    "${Constant.globalUrl}payment/phonepe_redirect", // Use the redirect URL for success detection
                failureUrl:
                    "${Constant.globalUrl}payment/phonepe_failure", // Example failure URL, adjust as per PhonePe's actual failure redirect
              ));

          if (paymentSuccess == true) {
            ShowToastDialog.showToast("Payment Successful!!".tr);
            walletTopUp(); // Update wallet on success
          } else {
            ShowToastDialog.showToast("Payment Unsuccessful!!".tr);
          }
        } else {
          ShowToastDialog.showToast(
              "Failed to initiate PhonePe payment: ${responseData['message'] ?? 'Unknown error'}"
                  .tr);
        }
      } else {
        log("PhonePe API Error: ${response.statusCode} - ${response.body}");
        ShowToastDialog.showToast(
            "Failed to initiate PhonePe payment. Please try again.".tr);
      }
    } catch (e, s) {
      log("PhonePe Payment Exception: $e \n$s");
      ShowToastDialog.closeLoader();
      ShowToastDialog.showToast(
          "An error occurred during PhonePe payment. Please try again.".tr);
    }
  }
