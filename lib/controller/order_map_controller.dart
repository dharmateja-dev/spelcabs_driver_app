...
class OrderMapController extends GetxController {
  ...
  RxDouble totalExcludingTax = 0.0.obs; // New variable for subtotal before taxes
  ...
  double _estimatedTimeInMinutes = 0.0;

  @override
  void onInit() {
    AppLogger.debug("OrderMapController onInit called.", tag: "OrderMapController");
    if (Constant.selectedMapType == 'osm') {
      ...
      AppLogger.info("OSM map controller initialized.", tag: "OrderMapController");
    }
    addMarkerSetup();
    getArgument();

    String? orderId = Get.arguments?['orderModel'];
    AppLogger.info("Order ID from arguments: $orderId", tag: "OrderMapController");
  ...
  @override
  void onClose() {
    AppLogger.debug("OrderMapController onClose called.", tag: "OrderMapController");
  ...
  getArgument() async {
    ...
    if (argumentData != null) {
      ...
      AppLogger.info("Order data loaded. Offer rate: ${orderModel.value.offerRate}", tag: "OrderMapController");
      if (Constant.selectedMapType == 'google') {
        getPolyline();
        AppLogger.debug("Getting polyline for Google Maps.", tag: "OrderMapController");
      }
    } else {
      AppLogger.warning("No arguments received for OrderMapController.", tag: "OrderMapController");
    }

    FireStoreUtils.fireStore.collection(CollectionName.driverUsers).doc(FireStoreUtils.getCurrentUid()).snapshots().listen((event) {
      if (event.exists) {
        driverModel.value = DriverUserModel.fromJson(event.data()!);
        AppLogger.info("Driver model updated from stream: ${driverModel.value.fullName}", tag: "OrderMapController");
      } else {
        AppLogger.warning("Driver document does not exist for current UID: ${FireStoreUtils.getCurrentUid()}", tag: "OrderMapController");
      }
    }, onError: (error) {
      AppLogger.error("Error fetching driver data in stream: $error", tag: "OrderMapController", error: error);
    });
    isLoading.value = false;
    update();
    AppLogger.debug("OrderMapController isLoading set to false.", tag: "OrderMapController");
  }

  getData(String id) async {
    AppLogger.debug("getData called for order ID: $id", tag: "OrderMapController");
    await FireStoreUtils.getOrder(id).then((value) {
      if (value != null) {
        orderModel.value = value;
        AppLogger.info("Order data retrieved for ID: $id", tag: "OrderMapController");
      } else {
        AppLogger.warning("Order data not found for ID: $id", tag: "OrderMapController");
      }
    }).catchError((error) {
      AppLogger.error("Error retrieving order data for ID $id: $error", tag: "OrderMapController", error: error);
    });
  }
  ...
  addMarkerSetup() async {
    ...
    try {
      if (Constant.selectedMapType == 'google') {
        ...
        AppLogger.info("Google Maps marker icons loaded.", tag: "OrderMapController");
      } else {
      ...
      AppLogger.error("Error setting up marker icons: $e", tag: "OrderMapController", error: e, stackTrace: s);
    }
  }
    ...
    if (orderModel.value.sourceLocationLAtLng != null && orderModel.value.destinationLocationLAtLng != null) {
      ...
      try {
        ...
        if (result.points.isNotEmpty) {
          ...
          AppLogger.info("Polyline points generated: ${polylineCoordinates.length}", tag: "OrderMapController");
        } else {
          AppLogger.warning("No polyline points found: ${result.errorMessage}", tag: "OrderMapController");
        }
      } catch (e, s) {
        AppLogger.error("Error getting route between coordinates: $e", tag: "OrderMapController", error: e, stackTrace: s);
      }
      ...
      AppLogger.debug("Markers added for Google Maps.", tag: "OrderMapController");
    } else {
      AppLogger.warning("Source or destination coordinates are null, cannot get polyline.", tag: "OrderMapController");
    }
  }
  ...
  movePosition() async {
    ...
    AppLogger.debug("Calculated distance: $distance miles, center: $center", tag: "OrderMapController");

    double radiusElevated = (distance / 2) + ((distance / 2) / 2);
    double scale = radiusElevated / 500;

    zoomLevel = 5 - log(scale) / log(2);
    AppLogger.debug("Calculated zoom level: $zoomLevel", tag: "OrderMapController");

    final GoogleMapController controller = await mapController.future;
    controller.moveCamera(CameraUpdate.newLatLngZoom(center, zoomLevel));
    AppLogger.info("Google Map camera moved to center with zoom level: $zoomLevel", tag: "OrderMapController");
  }

  _addPolyLine(List<LatLng> polylineCoordinates) {
    AppLogger.debug("_addPolyLine called with ${polylineCoordinates.length} points.", tag: "OrderMapController");
  ...
  addMarker(LatLng? position, String id, BitmapDescriptor? descriptor) {
    ...
    AppLogger.debug("Marker added: $id at ($position)", tag: "OrderMapController");
  }

  void getOSMPolyline(themeChange) async {
    AppLogger.debug("getOSMPolyline called with themeChange: $themeChange", tag: "OrderMapController");
    try {
      if (orderModel.value.sourceLocationLAtLng != null && orderModel.value.destinationLocationLAtLng != null) {
        setOsmMarker(
        ...
        AppLogger.info("OSM road drawn. RoadInfo: ${roadInfo.value.distance}m", tag: "OrderMapController");
        ...
        AppLogger.warning("Source or destination coordinates are null for OSM polyline.", tag: "OrderMapController");
      }
    } catch (e) {
      AppLogger.error('Error drawing OSM polyline: $e', tag: "OrderMapController", error: e);
    }
  }

  Future<void> updateCameraLocation({required GeoPoint source, required GeoPoint destination}) async {
    AppLogger.debug("updateCameraLocation (OSM) called with source: $source, destination: $destination", tag: "OrderMapController");
    BoundingBox bounds;
    ...
    AppLogger.debug("Calculated OSM BoundingBox: $bounds", tag: "OrderMapController");
    await mapOsmController.zoomToBoundingBox(bounds, paddinInPixel: 300);
    AppLogger.info("OSM map zoomed to bounding box.", tag: "OrderMapController");
  }

  setOsmMarker({required GeoPoint departure, required GeoPoint destination}) async {
    AppLogger.debug("setOsmMarker called with departure: $departure, destination: $destination", tag: "OrderMapController");
    if (osmMarkers.containsKey('Source')) {
      await mapOsmController.removeMarker(osmMarkers['Source']!);
      AppLogger.debug("Removed existing 'Source' OSM marker.", tag: "OrderMapController");
    }
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      await mapOsmController
        ...
        AppLogger.info("'Source' OSM marker added at $departure.", tag: "OrderMapController");
      }).catchError((error) {
        AppLogger.error("Error adding 'Source' OSM marker: $error", tag: "OrderMapController", error: error);
      });

      if (osmMarkers.containsKey('Destination')) {
        await mapOsmController.removeMarker(osmMarkers['Destination']!);
        AppLogger.debug("Removed existing 'Destination' OSM marker.", tag: "OrderMapController");
      }
      ...
      await mapOsmController
        ...
        AppLogger.info("'Destination' OSM marker added at $destination.", tag: "OrderMapController");
      }).catchError((error) {
        AppLogger.error("Error adding 'Destination' OSM marker: $error", tag: "OrderMapController", error: error);
      });
    });
  }
}
